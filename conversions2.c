...
/*   Updated: 2019/11/10 15:42:13 by esoulard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_putlnbr_base(unsigned long long lnbr, char d, char *str)
{
	...
	i = 1;
	size = 1;
	while ((lnbr / i) >= (unsigned long long)ft_strlen(base_are_belong(d)))
	{
		i = i * (unsigned long long)ft_strlen(base_are_belong(d));
		size++;
	}
	...
	while (i > 0)
	{
		kazbaz = (lnbr / i) % (unsigned long long)ft_strlen(base_are_belong(d));
...
char	*convert_addr(char *ret, va_list ap)
{
	...
	{
		if (!(ret = ft_malloc("char", (ft_strlen(addr) + 5))))
			return (NULL);
		i = -1;
		j = 0;
		ret[j++] = 'R';
...
char	*convert_unsigned_hexa(char *ret, va_list ap, char d)
{
	...
	{
		if (!(ret = ft_malloc("char", (ft_strlen(str) + 3))))
			return (NULL);
		ret[0] = 'R';
		ret[1] = '1';
		ret[2] = '\0';
		ft_strlcat(ret, str, (ft_strlen(str) + 3));
		ft_free(str);
	}
	return (ret);
}
